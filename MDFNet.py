...
class MDFNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return output, x0_c, x1_c, x2_c, x3_c, x4_c, x4_2

class MDFNetFFT(nn.Module):
    def __init__(self, in_ch=3, out_ch=1):
        super(MDFNetFFT, self).__init__()
    ...
    def forward(self, x):
        ...
        x3_3 = torch.cat([self.pool(x2_4), x3_1, _x3_c], 1)
        x3_3 = self.conv3_3(x3_3)
        x4_2i = torch.cat([self.pool(x3_3), x4_0, _x4_c], 1)
        x4_2i = self.conv4_2(x4_2i)

        phase, magnitude = self.fft(x4_2i )
        batch_size, channels, height, width = x4_2i .size()
        white_noise = torch.randn((batch_size, 1, height, width)).to(x4_2i.device).detach()
        # noise = torch.normal(0.75, 0.5625, phase.shape, device=x4_2i.device)
        scaled_white_noise = (1 + 0.5 * white_noise).clip(min=0)
        noised_magnitude = scaled_white_noise*magnitude
        x4_2 = self.ifft(noised_magnitude, phase)
        x4_2 = x4_2 .real
        ...
        output = self.final(x0_5)
        return output, x0_c, x1_c, x2_c, x3_c, x4_c, x4_2i
