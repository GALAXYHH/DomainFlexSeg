...
from evaluation import get_accuracy, get_sensitivity, get_specificity
from MDFNet import MDFNet, MDFNet_S
import torch.nn as nn
from losses.loss import SoftDiceLoss
import csv
import argparse
from algs.MySOTA import *

parser = argparse.ArgumentParser()
parser.add_argument('--time', type=int, default=1, help='timestamp')
parser.add_argument('--dataset_name', type=str, default='BUS_ABCD', help='dataset_name')
parser.add_argument('--dataset_test_name', type=str, default='BUS_E', help='dataset_test_name')
...
parser.add_argument('--save_path', type=str, default='./model/base/', help='path of save')
parser.add_argument('--img_size', type=int, default=320, help='input size')
parser.add_argument('--max_epochs', type=int, default=3000, help='maximum epochs to train')
...
def train(train_loader, model, criterion, optimizer, epoch, params):
    ...
    for i, (images, target) in enumerate(stream, start=1):
        ...
        size = images.size(0)
        output, x0_c, x1_c, x2_c, x3_c, x4_c, _ = model(images)
        loss = criterion(output, target) + criterion(x0_c, target) + criterion(Up2(x1_c), target) +  criterion(Up4(x2_c), target) + criterion(Up8(x3_c), target) + criterion(Up16(x4_c), target)
...
def validate(val_loader, model, criterion, epoch, params):
    ...
    with torch.no_grad():
        for i, (images, target) in enumerate(stream, start=1):
            ...
            target = target.to(params["device"], non_blocking=True).float().unsqueeze(1)
            size = images.size(0)
            output1, x0_c, x1_c, x2_c, x3_c, x4_c, _ = model(images)
            output2, x0_c, x1_c, x2_c, x3_c, x4_c, _ = model(torch.flip(images, [0, 3]))
            output2 = torch.flip(output2, [3, 0])
            output3, x0_c, x1_c, x2_c, x3_c, x4_c, _ = model(torch.flip(images, [0, 2]))
...
def train_and_validate(model, train_dataset, val_dataset, params):
    ...
    for epoch in range(1, params["epochs"] + 1):
        if (epoch+1)*len(train_loader) > max_iterations:
                break
        train(train_loader, model, criterion, optimizer, epoch, params)
        dice = validate(val_loader, model, criterion, epoch, params)
        if dice <= max_dice:
            print('\nEpoch: {}, lr: {:.6f},  Dice: {:.4f}'.format(epoch, exp_lr_scheduler.get_last_lr()[0], max_dice))
        else:
            max_dice = dice
            print(
                '\nEpoch: {}, lr: {:.6f},  Dice: {:.4f}, model saved!'.format(epoch, exp_lr_scheduler.get_last_lr()[0],
                                                                              max_dice))
...
if __name__ == "__main__":
    ...
    dataset_test = args.dataset_test_name
    mean = [0.31993517, 0.32047412, 0.32845072]
    std = [0.20349016, 0.20342242, 0.20599243]
    ...
    train_lst = args.data_path + dataset + '/train' + '.txt'
    val_lst = args.data_path + dataset + '/val' + '.txt'
    test_lst = args.data_path + dataset_test + '/all' + '.txt'
    model_root = args.save_path + dataset + '_' + net + '_time' + str(args.time) + '.pth'
    ...
    print('{} {} time {}'.format(dataset, net, args.time))
    model = create_model(params)
    train_and_validate(model, train_dataset, val_dataset, params)

    test_dataset = BCDataset(args.data_path, test_lst, transform=val_transform, inf=True)
    ...
    dice, iou, acc, sensitivity, specificity = test(test_loader, model, params)
    print('Dataset: {}, Model: {}, Dice: {:.4f}, IoU: {:.4f}, ACC: {:.4f}, Sen: {:.4f}, Spe: {:.4f}'.format(dataset,
                                                                                                            net,
                                                                                                            dice,
                                                                                                            iou,
                                                                                                            acc,
